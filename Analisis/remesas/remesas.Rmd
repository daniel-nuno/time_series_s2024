...
```{r}
(fit_sn %>% forecast(h = tstng_prds) %>% accuracy(test))
```
...
models_tst <- fit_sn %>% forecast(h = tstng_prds)
mape_sn <- (fit_sn %>% forecast(h = tstng_prds) %>% accuracy(test))$MAPE
snaive_plot <- models_tst %>% autoplot(filter_index(data, "2018 Jan" ~ .)) +
...
gdp_us
```
### Convertir el cambio porcentual a base 100 con la suma acumulada de los cambios porcentuales
...
### Gráfica de ambas series a través del tiempo
...
### Gráfica de dispersión (correlación) entre ambas series
...
### Zoom a la gráfica después de 2018
...
```
Los residuales estan autocorrelacionados, no son homocedástico y la media de los errores no es cero.

### Gráfica de residuales contra valores ajustados.
También sirve para identificar atípicos

```{r}
augment(fit_lm) %>% ggplot(aes(x=.fitted, y=.resid)) + geom_point() + labs(x="Fitted", y="Residuals")
```

Los datos de ambas variables no son estacionarios, es decir tienen tendencia. Y puede ser que ninguna tenga relación con la otra ni sea causal de la otra.
...
Para estacionalidad trimestral calcula 3 variables.
Para estacionalidad diaria calcula 23 variables.
Para estacionalidad semanal calcula 51 variables.

`TSLM` utiliza el argumento `season()` para indicar la estacionalidad.

### Atipicos/Spike/Intervention variable

Con lo observado en la gráfica y lo comentado anteriormente, las fechas de importancia para esta serie de tiempo son la crisis hipotecaria y la crisis covid.
Definir una columna de 0s, exepto entre las fechas indicadas (diciembre 2007 - junio 2009).
Definir una columna de 0s, exepto entre las fechas indicadas (febrero 2020 - mayo 2020).
...
Usa los valores pasados cómo predictor de X. Hipótesis: Las remesas (o el cambio porcentual) enviadas en el pasado influyen en el futuro. Revisa la autocorrelación.
...
report(fit_es)
```
Alpha es de 0.58, lo que indica que la última observación no es la más importante y las pasadas observaciones tienen un peso significativo
...
Usando ETS sin especificar ningún parámetro, el modelo se ajusta basado en el el menor error. Esta optimización regresa como resultado un modelo con errores multiplicativos, tendencia aditiva y estacionalidad multiplicativa.
El sigma^2 (0.0048) y AIC (5188) indican que es el mejor modelo utilizando ETS.
...
Utilizando ARIMA, sin especificar ningún parámetro obtenemos.
...
```{r}
train_gdp = train_gdp %>% mutate(diff = difference(log(remesas)))

fit_din = train_gdp %>% model(TSLM(diff ~ crisisc + crisish))
...
```{r}
fit_din = train_gdp %>% model(ARIMA(diff ~ crisisc + crisish))
...
```
```{r}
fit_din = train_gdp %>% model(ARIMA(log(remesas) ~ crisisc + crisish))

report(fit_din)
fit_din %>% gg_tsresiduals()
```
La última regresión dinámica, utilizando las remesas transformadas matemáticamente y los regresores cómo valores atípicos dan un buen resultado de sigma^2 (0.0049), AIC (-825) y log likelihood (418). Por lo tanto, de los tres modelos revisados para una regresión dinámica nos quedamos esta.
...
```{r}
fc_lm = forecast(fit_lm, new_data = test_gdp[c("gdp", "date", "crisish", "crisisc", "lag1")])
fc_lm %>% accuracy(test_gdp)
```
```{r}
fit_es %>% forecast(h=tstng_prds) %>% accuracy(test)
```
```{r}
fit_arima %>% forecast(h=tstng_prds) %>% accuracy(test)
```

```{r}
fc_din = forecast(fit_din, new_data = test_gdp[c("date", "crisish", "crisisc")])
fc_din %>% accuracy(test_gdp)
```
El modelo ARIMA es el que tuvo mejor resultado comparando con RMSE. Por lo tanto se realizan los pronosticos con el modelo.
